...
def worker():
	...
	"myfile" in the S3 bucket "worker".
...
def tasks():
	'''
	Opens the file "tasks" in the S3 bucket "worker", reads whitespace delimited file names, and for each
...
def read_s3(file_name):
	try:
		return boto3.resource("s3").Object(bucket_name = "worker", key = file_name).get()["Body"].read().decode("utf-8")
	except botocore.exceptions.ClientError as e:
		return str(e)

def write_s3(file_name, message):
	try:
		boto3.resource("s3").Bucket("worker").put_object(Key = file_name, Body = str(message))
		return "Message sent"
	except botocore.exceptions.ClientError as e:
		return str(e)
